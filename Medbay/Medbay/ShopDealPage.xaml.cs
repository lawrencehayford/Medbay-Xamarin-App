...
namespace Medbay
{
	...
	public partial class ShopDealPage : ContentPage
	{
        ...
        private void  populate(string content, string quanty, string amount, string image, string id)
        {

            itemList = content + "~" + quanty + "~" + amount + "~" + image + "~" + id + "`";
            quant = 0;
            Int32.TryParse(quanty, out quant);

            //adding to list
            Product.Add(content);
            ProductId.Add(id);
            ProductUnitPrice.Add(Decimal.Parse(amount));
